...
- Removed documentation examples for `Picklist` small and xsmall variants.
...
- `Picklist`, `Pickitem`, and related components
  ...
  - Size variants `small` and `xsmall` now have a `sunsetting` flag due to poor touch interaction potential.
...
- Rounded `min-height` property generated by `Positioning` (for `Dropdown`) to fix blurry dropdown on position change (sub-pixel rendering in Chrome).
- Reverse tabbing (`shift` + `tab`) through modals with `restrictFocus` no longer traps the focus.
- Updated storybook to latest version 5.
- Added warnings for pickitem props that should not be used with horizontal picklists.
