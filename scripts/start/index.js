...
const buildKss = require(path.resolve(rootDirectory, 'scripts', 'build', 'kss'));
const buildXui = require(path.resolve(rootDirectory, 'scripts', 'build', 'xui.js'));
...
function watchBoth() {
  ...
  Promise.all([buildKss(), buildXui(), removeDistDocsReact()])
    .then(() => {
