...
export default class XUIAvatarGroup extends PureComponent {
	render() {
		...
		const extraChildCount = (maxAvatars && childCount > maxAvatars)
			? (childCount - maxAvatars) + 1
			: 0;
		const lastChildIndex = extraChildCount ? maxAvatars - 1 : childCount;
		let variant = 'standard';

		const childrenToRender = (avatarSize || extraChildCount)
		...
		const counter = extraChildCount
			? <XUIAvatarCounter variant={variant} count={extraChildCount} size={avatarSize} />
			: null;

		return (
			<div data-automationid={qaHook} className={cn(classNames.group, className)}>
				{childrenToRender}
				{counter}
			</div>
		);
