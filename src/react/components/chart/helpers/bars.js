...
const findMaxChartStackQuantity = barsData =>
  barsData.reduce((acc, { y }) => Math.max(acc, y.length), 0);
...
const createBarStats = ({ barsData, xAxisVisibleItems, viewportWidth, hasPaginationRaw }) => {
  ...
  const limitWithUpperAndLowerThreshold = baseWidth =>
    baseWidth > BAR_MAX_WIDTH ? BAR_MAX_WIDTH : limitWithLowerThreshold(baseWidth);
  ...
  const totalBarsPerPanel =
    hasPaginationRaw || !hasMultiplePanels
...
const createBarColorStacks = ({ barsData, custom, base }) => {
  ...
  return createArray(maxStacks).map((_, index) => (custom && custom[index]) || base[index % 2]);
};

const createActiveBars = (activeBarsRaw, barsData) =>
  Object.keys(activeBarsRaw)
...
const enrichParams = (state, props, chartTheme) => {
  ...
  const barsData = isBarStacked
    ? barsDataRaw.map(bar => ({ ...bar, y: bar.y || [0] }))
    : barsDataRaw.map(bar => ({ ...bar, y: [bar.y || 0] }));

  // Key...
  const hasKey =
    keyLabel && keyLabel.filter(key => key).length === findMaxChartStackQuantity(barsData);
  ...
  const { barsWidth, barWidth, barMaxValue, barMinValue, barViewports } = createBarStats({
    barsData,
    xAxisVisibleItems,
    viewportWidth,
    hasPaginationRaw,
  ...
  const createYAxisLabelFormat =
    createYAxisLabelFormatRaw || createYAxisLabelFormatThunk(yAxisTickValues);
  ...
  const createBarToolTipMessage =
    createBarToolTipMessageRaw ||
