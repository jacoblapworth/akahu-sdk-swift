...
export const createYAxisLabelFormatThunk = ({ yAxisMaxValue }) => {
	...
	return rawLabel => {
		...
		const factor = Math.pow(10, (decimalLength + 1)); // eslint-disable-line no-restricted-properties
...
export const createYAxisTickValues = ({ yAxisHeight, maxValue, minValue }) => {
	...
	const increment = hasAnyYValue
		? forceValuePositive(minValue) + (yAxisMaxValue / totalLabels)
