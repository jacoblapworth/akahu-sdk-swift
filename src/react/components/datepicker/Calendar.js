...
export default class Calendar extends PureComponent {
	...
	render() {
		...
		const modifiers = selectedRange
			? getRangeModifiers(selectedRange, this.state.hoverDate, this.isDayDisabled)
		...
		if (isPartialRange(selectedRange)) {
			selectedDays.push(selectedRange.from);
		}
		if (isRangeComplete(selectedRange)) {
...
Calendar.propTypes = {
	...
	/** @prop {Date} If you only want to display a single selected day without allowing the user to select a date range, pass that Date in here. */
