...
function getTriggerAlignmentStyles({
	...
	const canAlignLeftEdge = popupRect.width <= (
		(spaceRightOfTrigger + triggerRect.width)
		- viewportGutter - triggerDropdownGap
	);
	...
	const marginLeft = !isNotResponsive && isNarrowViewport()
		? '0px'
...
class Positioning extends PureComponent {
	...
	positionComponent = () => {
		...
		if (parentRef) {
			...
			if (isBaseRendered(popupRect)) {
				const styles = (!isNotResponsive && isNarrowViewport())
	...
	calculateMaxHeight = () => {
		...
		if (verge.inViewport(triggerDOM)) {
			if (!isNotResponsive && isNarrowViewport()) {
				...
				const spaceAboveTrigger = calcSpaceAbove(triggerRect);
				const spaceBelowTrigger = calcSpaceBelow(triggerRect);
				const availableSpace = Math.max(spaceAboveTrigger, spaceBelowTrigger)
					- viewportGutter - triggerDropdownGap;
	...
	getStyles = () => {
		...
		const width = shouldMatchTriggerWidth
	...
	render() {
		...
		const clonedChildren = isVisible && positioned
