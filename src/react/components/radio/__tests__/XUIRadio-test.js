...
describe('XUIRadio', () => {
	...
	it('should use the xui-icon class on the SVG element', () => {
		...
		expect(component.find('svg').hasClass('xui-icon')).toBeTruthy();
	});
	...
	it('should define role as presentation on each path element', () => {
		...
		expect(component.find('svg').childAt(0).prop('role')).toEqual('presentation')
		expect(component.find('svg').childAt(1).prop('role')).toEqual('presentation')
		expect(component.find('svg').childAt(2).prop('role')).toEqual('presentation')

	});


	// Icon combinatorics 1/9 (main: undefined, check: undefined)
	it('should use the default iconMainPath and iconCheckPath if both are undefined', () => {
		const component = shallow(<XUIRadio onChange={NOOP} />);

		expect(component.find('svg').childAt(0).prop('d')).toEqual(radioMain);
		expect(component.find('svg').childAt(1).prop('d')).toEqual(radioMain);
		expect(component.find('svg').childAt(2).prop('d')).toEqual(radioCheck);
	});


	// Icon combinatorics 2/9 (main: undefined, check: null)
	it('should use the default iconMainPath and iconCheckPath if iconMainPath is undefined and iconCheckPath is null', () => {
	...
	it('should use the default iconMainPath and iconCheckPath if iconMainPath is null and iconCheckPath is undefined', () => {
	...
	it('should use the default iconMainPath and iconCheckPath if both are null', () => {
