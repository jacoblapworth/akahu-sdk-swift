...
src/react/umd.js
*/
const umdFile = path.join(__dirname, '..', 'src', 'react', 'umd.js');
const dir = path.join(__dirname, '..', 'src', 'react');

// Find all public modules in src/react and build up a map from moduleName => importPath
const moduleMap = {};
fs.readdirSync(dir)
	.map(file => path.join(dir, file))
	.filter(file => fs.statSync(file).isFile() && path.extname(file) === '.js' && path.basename(file) !== 'umd.js')
	.forEach(file => {
